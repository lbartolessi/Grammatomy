...
- **Mutation Authority:** Structural mutations (detach, reabsorb, prune) are calculated exclusively on the Server (Core) to ensure consistency with the validation logic. The Client (Studio) is a renderer and command issuer, never a logic executor for structural changes.
- **Platonic Architecture (The Cave Metaphor):** The Backend (AnyTree) is the World of Ideas—the absolute, immutable truth of the structure. The Frontend (Studio) is "the Cave" —a mere projection of shadows (renderings) that allows humans to interact with the truth but never defines it. All structural logic and sanitization must occur in the Backend.
- **Ephemeral Projections (Master Map Principle):** Complex, read-only views like the "Master Map" are treated as ephemeral, stateless projections. They are generated on-demand from the source-of-truth data (the individual editable trees) and are immediately discarded upon any navigation action. This prevents state corruption by ensuring that read-only, stitched-together views can never be saved back over the canonical, fragmented data.
- **Cinematic Transitions (Cognitive Continuity):** UI transitions between complex data views (e.g., syntax trees) must be animated. Instantaneous "teleportation" is avoided as it causes cognitive dissonance. Smooth, animated transitions (pan, zoom, cross-fade) provide spatial awareness, helping the user understand the path taken through the data, thus making the application psychologically friendly and reducing fatigue.
