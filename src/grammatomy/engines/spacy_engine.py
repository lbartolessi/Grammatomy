...
def _patch_transformers():
    ...
    """
    try:
        from transformers import T5Tokenizer, T5TokenizerFast, XLMRobertaTokenizer, XLMRobertaTokenizerFast
...
class SpacyEngine:
    ...
    @classmethod
    def get_tree(
        cls, text: str, lang: str = "es", model_package: str = "default", use_gpu: bool = False
    ) -> Optional[Node]:
        """
        ...
        config_model = hf_model
        if "/" in hf_model and not os.path.exists(hf_model):
            try:
                print(f"Downloading '{hf_model}' from Hugging Face...")
                config_model = snapshot_download(repo_id=hf_model)
            except Exception as e: # pragma: no cover
                logger.error(f"HF download failed for {hf_model}: {e}")
                raise RuntimeError(f"Could not download model '{hf_model}' from Hugging Face.") from e
        ...
        if cache_key not in cls._pipelines:
            ...
            try:
                ...
                logger.warning(f"Base spaCy model for '{lang}' not found. Using blank model.")
                nlp = spacy.blank(lang)
            ...
            if use_gpu:
                benepar.mkdtemp = lambda: None # Hack to prevent some benepar issues on GPU sometimes
                # Note: Benepar GPU usage is handled by torch global state usually
            
            nlp.add_pipe("benepar", config={"model": config_model})
            cls._pipelines[cache_key] = nlp
